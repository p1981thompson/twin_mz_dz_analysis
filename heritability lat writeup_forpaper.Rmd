...
library(officer)
library(umx)
# install.OpenMx("NPSOL") #From Tim B: can't get this to work! but probably unimportant
# umx_set_optimizer('NPSOL') # from Tim B who says probably unimportant!
...
**Results**
For the two handedness measures, heritability estimates were consistent with prior research: .23 and .22 respectively for the EHI and QHP on nonparametric analysis. For the language laterality index, however, the twin-cotwin correlations were very close to zero for both MZ and DZ twins, and the heritability estimate was zero. For the smaller sample with language laterality data, there was 80% power to detect heritability of .215 in an AE model.

**Conclusions** 
A single study showing negligible heritability for language laterality cannot rule out a genetic effect on language lateralisation. It is possible that the low twin-cotwin correlations were affected by noisy data: although the split-half reliability of the laterality index was high (.85), we did not have information on test-retest reliability in children, which is likely to be lower. The power of the study was not high enough to rule out the possibility that there is significant heritability for language lateralisation, but results indicate that the heritability of this trait is low at best. Stochastic variation in neurodevelopment appears to play a major role in determining cerebral lateralisation.
...
mycols<-c("ehp_handedness", "qhp_handedness", "lat_index","li_cat")
...
mzData = subset(df, Zygosity=='MZ')
dzData = subset(df, Zygosity=='DZ')
...
Bates, T. C., Maes, H. H., & Neale, M. C. (2017). umx: Twin and Path-based Structural Equation Modeling in R. PeerJ Preprints, 5, e3354v3351. doi:10.7287/peerj.preprints.3354v1
