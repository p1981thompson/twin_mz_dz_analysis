...
tab2f<-tab2f[,1:2]
tab2f<-data.frame(tab2f)
colnames(tab2f)<-c('All','With_fTCD')
nr<-nrow(tab2f)
tab2f[(nr+1),]<-colSums(tab2f)
...
#tab2f %>% kable(booktabs = T, caption = mycaption) %>% 
#  kable_styling()
tab2f<-cbind(forflex,tab2f)
tab2f[,2]<-as.integer(tab2f[,2])
tab2f[,3]<-as.integer(tab2f[,3])
flextable(tab2f,col_keys=names(tab2f))
...
thisrow<-0
for (myn in 2:3){ #estimate for both sample sizes, as shown in cols 2 and 3 of tab2f
...
![Figure 1. Density plots for handedness and language laterality indices](plots/combined_densities.png)
...
![Figure 2. Scatterplots by zygosity for handedness and language laterality indices. The data from handedness measures are jittered; Spearman correlation coefficients are shown.](plots/combined_plot.png)
...
```
The OpenMx package was used to run an AE model with the data from the two handedness tasks and the language laterality task. It was anticipated that the model would not fit with the data because (a) the handedness data were highly non-normal, and (b) the correlations for the language laterality index were close to zero. However, a good fit was obtained for all three measures.  Heritability estimats for the two handedness measures were statistically significant and compatible with those obtained in previous studies, with values of a^2^ of `r aatable[1,6]` for the Edinburgh Handedness Inventory and `r aatable[2,6]` for the Quantification of Hand Preference task. For the language laterality index, the estimated value of a^2^ was zero.
...
ft <-flextable(data = aatable) %>% 
  theme_booktabs() %>% 
  set_header_labels( Measure ="Measure", N_MZ="N MZ", N_DZ="N DZ", a="a", a_SE="a (SE)", a_sqr="a^2") 
...
singledata[, ordDVs] <- mxFactor(singledata[, ordDVs], levels = c(0:nlevels))
...
umxSummary(m1)
parameters(m1)
Qa<-as.numeric(unlist(parameters(m1))[46]) #hard coded to get the right index :-(
Qa2<-round(Qa^2,2)
if(i==1)
  {Ea<-as.numeric(unlist(parameters(m1))[24])#hard coded to get the right index :-(
Ea2<-round(Ea^2,2) #preserve initial model
 }
}
```
Because of concerns about the non-normality of the handedness data, analyses of the EHI and QHP were repeated using the umx package (Bates, 2018) to run an ordinal version of ACE analysis. This gave closely similar results, with values of a^2^ of `r Ea2` and `r Qa2` for EHI and QHP respectively.


```{r DFanalysis, echo=FALSE, include=FALSE}
...
Elefties<-filter(doubledata,ehp_handedness<6)
...
Qlefties<-filter(doubledata,qhp_handedness<11)
thisrow<-2
...
Drighties<-filter(doubledata,lat_index<0)
thisrow<-3
...
colnames(dftable)<-c('Measure','N_MZ','N_DZ','MZ_proband','DZ_proband',
                     'MZ_twin','DZ_twin','t','df','p')

flextable(dftable)

ft2<-flextable(data = dftable) %>% 
  theme_booktabs() %>% 
  set_header_labels( Measure ="Measure", N_MZ="N MZ", N_DZ="N DZ", MZ_proband="MZ proband mean (SD)", DZ_proband="DZ proband mean (SD)", MZ_twin= "MZ co-twin mean (SD)", DZ_twin="DZ co-twin mean (SD)", t="t",df= "df", p="p") 
...
Some genetic models of handedness have proposed two basic types of laterality genotype: one with a bias to left-sidedness for language and right-sidedness of handedness, with chance playing a major role in determining degree of handedness for either type. Failure to find specific genetic variants associated with non-right-handedness in large-scale genome-wide association studies has led researchers to propose that this phenotype may be the consequence of a wide range of disparate genetic mutations. If this model holds, then if we identify individuals with atypical handedness or language laterality, they would have a strong likelihood of carrying a specific mutation that reduces lateralisation. Unless the mutation arose post-fertilisation, then it would be shared in all MZ twins, but in only 50% of DZ twins. Thus if we select individuals with atypical laterality, MZ cotwins of these individuals should be similarly atypical, whereas DZ cotwins should be more likely to have typical laterality. This is the basic logic of the DeFries-Fulker (1988) method for analysing heritability of extreme scores. Although this can be tested in the OpenMx framework, the plausibility of such a model can be readily tested by selecting twins with an extreme score and then using a t-test to compare co-twin scores for MZ and DZ twins. Results of such an analysis are shown for all three phenotypes in Table 4, where the contrasts between MZ and DZ cotwins are not statistically reliable. These data must be interpreted with extreme caution because of the small sample sizes, but they do not lend any support to the idea that there is a distinct genetic origin of atypical laterality, either for handedness or for language laterality.
