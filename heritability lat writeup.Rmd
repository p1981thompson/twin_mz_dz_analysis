...
**Results**
For the two handedness measures, heritability estimates were consistent with prior research: .23 and .22 respectively for the EHI and QHP on nonparametric analysis. For the language laterality index, however, the twin-cotwin correlations were very close to zero for both MZ and DZ twins, and the heritability estimate was zero. Power to detect heritability of .25 was 91%.

**Conclusions** 
A single study showing negligible heritability for language laterality cannot rule out a genetic effect on language lateralisation. It is possible that the low twin-cotwin correlations were affected by noisy data: although the split-half reliability of the laterality index was high (.85), we did not have information on test-retest reliability in children, which is likely to be lower. Although the sample is small for genetic analysis, the power of the AE model is substantially higher than for a conventional model that includes a term for shared environment. These results raise the provocative possibility that individual differences in cerebral lateralisation for language may be caused solely by stochastic variation in neurodevelopment.
...
#Find the value of a2 where lower CI is just above 0 - this would be 95% power!
...
power.table<-mxPowerSearch(trueModel = ae, falseModel = nullModel, method = "ncp")
power.table
write_csv(power.table,'powertable.csv')

w<-max(which(power.table$N<smallN))
power.smallN25<-round(power.table[w,2],2)
w<-max(which(power.table$N<bigN))
power.bigN25<-round(power.table[w,2],2)


#Lines below:These precise power estimates seem wrong: the same regardless of N. Ignore for now.
#mypower.small<-mxPower(trueModel = ae, falseModel = nullModel, n=smallN/2)
#mypower.big<-mxPower(trueModel = ae, falseModel = nullModel, n=bigN/2)
#mypower.tiny<-mxPower(trueModel = ae, falseModel = nullModel, n=50)
```
## Data analysis
1. AE modeling 
...
A power analysis was conducted to estimate the power to detect heritability of .25 or more, using the mxPowerSearch function in OpenMx. This showed that for the larger sample of twins for whom handedness data were available, there is `r power.bigN25` power to detect heritability of .25 in an AE model, and for the smaller sample with language laterality data, power of `r power.smallN25`. Thus even the low heritability estimates reviewed in the Introduction, for handedness, structural asymmetry and dichotic listening, should be detectable with this sample size.
...
```{r batesmultivariate, echo=FALSE, include=FALSE}
...
```{r umxcompute,echo=FALSE, include=FALSE}
...
#Exploring data - not included in paper
...
## Further bits - just for historical interest, not used
