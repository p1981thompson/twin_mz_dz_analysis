...
tabnum<-1 #keeps track of table numbers. Table 1 is for literature review
```
...
Some genetic models of handedness have proposed two basic types of laterality genotype: one with a bias to left-sidedness for language and right-sidedness of handedness, with chance playing a major role in determining degree of handedness for either type. If this model holds, then if we identify individuals with atypical handedness or language laterality, they would have a strong likelihood of belonging to the unbiased group. MZ cotwins of these individuals should be similarly unbiased, whereas DZ cotwins should show less bias. This is the basic logic of the DeFries-Fulker (1988) method for analysing heritability of extreme scores. Although this can be tested in the OpenMx framework, the plausibility of such a model can be readily tested by selecting twins with an extreme score and then using a t-test to compare co-twin scores for MZ and DZ twins. Results of such an analysis are shown for all three phenotypes in Table 4. These data must be interpreted with extreme caution because of the small sample sizes, but they do not lend any support to the idea that there is a distinct genetic origin of atypical laterality, either for handedness or for language laterality.
...
Bishop, D. V. M. (1990). Handedness and developmental disorder. Oxford: Blackwell Scientific and Philadelphia: J.B. Lippincott.
