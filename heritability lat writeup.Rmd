...
mydata$taskssum_jitter<-jitter(mydata$taskssum)
...
data.short<-dplyr::select(mydata,record_id,fam_id,age_at_test,female,zygosity,myinclude,twin,n_trials, laterality_index,qhp_freq_r,ehp_handedness,lang_probs,lateralised_category,lat3,ehp_jitter,qhp_jitter,taskssum,taskssum_jitter)
...
Bishop (2013) suggested that inconsistency of laterality across different measures may be an indicator of underlying genotype. To test this idea, children were categorised as consistent or inconsistently lateralised: consistent lateralisation was coded if the children was either RRL or LLR on EHI, QHP and laterality index respectively, and inconsistent otherwise.
...
In a further analysis, children were categorised according to whether their laterality was consistent across the two handedness tasks and the language laterality task (i.e., either R for handedness and L for language, or vice versa). Concordance between twins relative to zygosity is shown in Table 2.

```{r latcats, echo=FALSE}
#Be warned - this is mind boggling because we are focusing both on concordance between two twins, AND on consistency of laterality across tasks.
#THis code is a v clunky way of creating a category reflecting concordance of both twins
#when categoised as RRL,LLR or inconsistent
doubledata$bothcat3<-NA #initialise
#We want to ignore the order of twins, so e.g. create categories where 1/3 and 3/1 the same
#Will end up with a 2-digit code for the pair, each digit reflecting RRL/LLR/inconsistent
w<-which(doubledata$lat32>doubledata$lat3)
doubledata$bothcat3[w]<-(10*doubledata$lat3[w])+doubledata$lat32[w]
w<-which(doubledata$lat32==doubledata$lat3)
doubledata$bothcat3[w]<-10*doubledata$lat3[w]+doubledata$lat32[w]
w<-which(doubledata$lat32<doubledata$lat3)
doubledata$bothcat3[w]<-10*doubledata$lat32[w]+doubledata$lat3[w]
doubledata$bothcat3<-as.factor(doubledata$bothcat3)
levels(doubledata$bothcat3)<-c('Concordant, RRL','Concordant, LLR','Discordant, RRL/inconsistent','Discordant, LLR/inconsistent','Concordant,inconsistent')
doubledata$MZDZ<-as.factor(doubledata$MZDZ)
levels(doubledata$MZDZ)<-c('MZ','DZ')
table(doubledata$bothcat3,doubledata$MZDZ,doubledata$twin)
```
##Session information
```{r sessinfo}
sessionInfo()
