...
data.short<-dplyr::select(mydata,record_id,fam_id,age_at_test,female,zygosity,n_language_low,myinclude,twin,n_trials, laterality_index,qhp_handedness,ehp_handedness,lang_probs,lateralised_category,lat3,ehp_jitter,qhp_jitter,taskssum,taskssum_jitter)
...
for (i in 1:2){
  ...
  rlat<-cor(tempdat[,mycols[j,1]],tempdat[,mycols[j,2]],method='spearman')
  mysub<-paste0('rho =',toString(round(rlat,3))) #text with correl coeff to 3 places
  #would like greek letter! expression(rho) did not work!
 ...
 #This is just to help visualise the plot: reassignment is not retained for later   analysis
  ...
  #------------------------------------------------------------------------------
  thisx<-tempdat1[,mycols[j,1]]
  thisy<- tempdat1[,mycols[j,2]]
  #attempt to see if inverting scale and taking exponential changes things
  # mymax<-max(c(thisx,thisy))-1
  # thisx<-exp(mymax-thisx)
  # thisy<-exp(mymax-thisy)
  #   rlat<-cor(thisx,thisy,method='spearman')
    
  mymain=paste0(mymain,'N = ',nrow(tempdat1),' pairs')
  plot(thisx, thisy,xlim=tasklims[j,1:2],ylim=tasklims[j,1:2],pch=pchcode[tempdat$gender2],
...
#dev.off() #needed to turn off printing to external pdf etc
```
...
shapiro.test(doubledata$laterality_index)
qqnorm(doubledata$laterality_index)
...
stargazer(aatable,type='text',title='Table 3: Heritability estimates for different measures:\n standard ACE ignoring nonnormality',summary=FALSE)
...
print('Polychoric correlations (rho)')
mypoly.mz
mypoly.dz
